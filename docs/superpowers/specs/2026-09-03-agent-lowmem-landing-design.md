...
2. Hero opening with the compact monospace tagline “More agents. Less RAM.” followed by a plain-language explanation that leads with “Native policy runner for memory-constrained Macs.” and ends with “Written in Rust.”
...
10. The footer closes with “Built for constrained machines.” and groups the source link above a clearly labelled theme selector. It does not duplicate the hero tagline or claim a project license until one is selected.
...
The interface uses a warm black background, ivory foreground, restrained neutral text, thin borders, and a narrow vertical reading column. Headlines, section titles, and section labels use a medium `500` weight; the primary action and inline product definition retain `600` as deliberate emphasis. A CSS-only lavender-to-indigo-to-cyan gradient accents only the wordmark cursor, status dot, primary-link underline, terminal prompt, ready state, and final `result` label. The final label uses a dedicated horizontal version of the gradient so its state reads clearly without changing the direction of the other accents. A light palette uses darker stops with the same hierarchy.
...
The browser selects the initial palette with `prefers-color-scheme`. A compact footer control grouped directly below the source link places its `theme:` label on one line and the `system / light / dark` choices on a separate aligned row. It may override the system choice for the current page using CSS `:has()`. Keeping it out of primary navigation preserves the header hierarchy and makes its purpose explicit. The radio group is accessible by keyboard and carries no script, local storage, cookie, request, or initial flash. It intentionally resets to `system` after reload; browsers without the selector support retain the valid system-selected theme. The light palette uses pure white for the page background and restrained neutral-gray surfaces.
...
The wordmark is textual: `agent_lowmem`. The primary action is an underlined text link rather than a bordered or filled button. A short entrance treatment may move the terminal once and then stop. The doctor and managed-run sequences may reveal each state once and must settle completely. The page performs no continuous animation after its initial presentation. `prefers-reduced-motion: reduce` removes the entrance treatment, skips the transient doctor sequence, and shows the completed managed-run sequence immediately.
